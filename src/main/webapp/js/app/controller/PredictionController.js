define([
	...
	var predict = function (data, chartDom, activeWindow, datesInterval, intervalCalendar, myLayout) {
	...
	controller.makeController = function (predictWindow, windowManager) {
		...
		$("#predictButton", activeWindow).click(function () {
			...
			if (config.optimizeSeries) {
				var optimizedYears = optimizationMap[config.historicalPriceFrequency];
				if (optimizedYears && config.historicalInterval.years > optimizedYears) {
					dhtmlx.message({
						type: "successMsg",
						text: "Historical market data period is optimized to " + optimizedYears + " years.",
						expire: 5000
					});
				}
			}
			;
			var warningMap = {
				"DAILY": function (historicalInterval, backwardPropagation, ifTrue, ifFalse) {
					if (historicalInterval.years >= 1 || backwardPropagation.years >= 1
							|| historicalInterval.months + backwardPropagation.months >= 12)
						ifTrue();
					else
						ifFalse();
				},
				"WEEKLY": function (historicalInterval, backwardPropagation, ifTrue, ifFalse) {
					if (historicalInterval.years + backwardPropagation.years >= 7)
						ifTrue();
					else
						ifFalse();
				},
				"TWO_WEEKS": function (historicalInterval, backwardPropagation, ifTrue, ifFalse) {
					if (historicalInterval.years + backwardPropagation.years >= 14)
						ifTrue();
					else
						ifFalse();
				},
				"MONTHLY": function (historicalInterval, backwardPropagation, ifTrue, ifFalse) {
					if (historicalInterval.years + backwardPropagation.years >= 30)
						ifTrue();
					else
						ifFalse();
				}
			};
			var alertFuntion = warningMap[config.historicalPriceFrequency];
			if (alertFuntion) {
				alertFuntion(config.historicalInterval, config.backwardAnalyticalPeriod, function () {
					dhtmlx.confirm({
						text: 'The calculation might take more than a few minutes.<br/> Do you want to continue?',
						cancel: 'NO',
						ok: 'YES',
						callback: function (btn) {
							if (btn === true)
							{
								predict(data, chartDom, activeWindow, datesInterval, intervalCalendar, myLayout);
							}
						}});
				}, function () {
					predict(data, chartDom, activeWindow, datesInterval, intervalCalendar, myLayout);
