...
public class EsgAdapter extends CalculationAdapter {
    ...
    public void loadFactors(FactorsSelectionConfig selectionConfig) throws InterruptedException {
        ...
        loadFactors(selectionConfig, marketFactorVo, false);
    }

    public void loadFactors(FactorsSelectionConfig selectionConfig, InstrumentMarketFactorVo marketFactor, boolean isSessionMarketFactorVo) throws InterruptedException {
        evalId = selectionConfig.getEvaluationIdVo();
        errors.clear();
        mfi = null;
        if (isSessionMarketFactorVo == false) {
            getMarketFactor(SeriesAdapter.MARKET_FACTORS.get(selectionConfig.getSeriesName() + "INEA"));
            marketFactor = marketFactorVo;
        }else{
            marketFactorVo = marketFactor;
        }
        if (!marketFactor.getInstrumentId().equals("dummy")) {
            loadMfi(marketFactor.getInstrumentId());
        }
        if (mfi == null) {
            errors.clear();
